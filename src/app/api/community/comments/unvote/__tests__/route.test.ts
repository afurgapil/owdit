/**
 * Tests for src/app/api/community/comments/unvote/route.ts
 */

const removeVote = jest.fn();
jest.mock('@/shared/lib/community/comments', () => ({
  communityComments: {
    removeVote: (...args: any[]) => removeVote(...args),
  },
}));
...
describe('community/comments/unvote route', () => {
  beforeEach(() => jest.clearAllMocks());

  test('401 on invalid nonce', async () => {
    const mod = await loadRoute();
    verifySignature.mockReturnValueOnce(true);
    consumeNonce.mockReturnValueOnce(false);
    const res = await mod.POST(buildPOST({ commentId: 'c', address: '0x1111111111111111111111111111111111111111', signature: 's', nonce: 'n', timestamp: Date.now() }));
    expect(res.status).toBe(401);
  });

  test('200 on success with removed flag', async () => {
    const mod = await loadRoute();
    verifySignature.mockReturnValueOnce(true);
    consumeNonce.mockReturnValueOnce(true);
    removeVote.mockResolvedValueOnce({ removed: true, delta: -1 });
    const res = await mod.POST(buildPOST({ commentId: 'c', address: '0x1111111111111111111111111111111111111111', signature: 's', nonce: 'n', timestamp: Date.now() }));
